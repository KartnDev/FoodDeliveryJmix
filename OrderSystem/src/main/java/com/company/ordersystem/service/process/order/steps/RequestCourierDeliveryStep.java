...
import com.company.ordersystem.client.CourierClient;
...
public class RequestCourierDeliveryStep extends AbstractTransactionalStep {
    private final Logger log = LoggerFactory.getLogger(RequestCourierDeliveryStep.class);

    private final CourierClient courierClient;
    private final OrderService orderService;

    protected RequestCourierDeliveryStep(SystemAuthenticator systemAuthenticator,
    ...
    protected void doTransactionalStep(DelegateExecution execution, OrderEntity order, SaveContext saveContext) {
        ...
        String result = systemAuthenticator.withUser(username,
                () -> courierClient.requestForCourier(orderService.convert(order), subjectToken));
        order.setStatus(DraftOrderStatus.COURIER_FINDING);
